...
def getValidRobotRadius():
    while True:
        try:
            robotRadius = int(
                input("Enter the radius of the robot as an integer from 1-12: ")
            )
        except ValueError:
            print(
                "Sorry, results invalid. Please try again, entering the input as an integer between 1-12. "
            )
            continue
...
def getValidCoords(type, maze, robotRadius, x2Maze):
    while True:
        try:
            ...
            continue
        if (
            coords[0] < 0 + robotRadius
            or coords[0] > 600 - robotRadius
            or coords[1] < 0 + robotRadius
            or coords[1] > 250 - robotRadius
        ):
            print(
                "Sorry, results invalid. Please try again, entering two integer inputs within the maze space. "
            )
    ...
    while True:
        try:
            theta = int(
                input(
                    "Enter "
                    + type
                    + " node orientation as an integer between 0-360, using increments of 30 deg: "
                )
            )
            if theta % 30 != 0:
                raise ValueError
            ...
            print(
                (
                    "Sorry, entry invalid. Please try again, entering an integer input between 0-360 in increments of 30. "
                )
            )
            continue
        if not searchNode((coords, theta), x2Maze):
            if type == "start":
                print(
                    (
                        "Sorry, entry invalid. Please try again, entering an integer input between 0-360 in increments of 30, oriented toward the center of the mazespace. "
                    )
                )
                continue
            else:
...
def getValidStepSize():
    while True:
        try:
            stepSize = int(
                input("Enter the step size of the robot as an integer from 1-10: ")
            )
        except ValueError:
            print(
                "Sorry, results invalid. Please try again, entering the input as an integer between 1-10. "
            )
            continue
...
# calculate the euclidean distance between the current node to goal node
...
def drawMaze(robotRadius):
    ...
    cv2.rectangle(
        maze,
        pt1=(robotRadius, robotRadius),
        pt2=(mazeSize[1] - robotRadius, mazeSize[0] - robotRadius),
        color=(255, 255, 255),
        thickness=-1,
    )
    ...
    cv2.rectangle(
        maze,
        pt1=(100 - robotRadius, 0),
        pt2=(150 + robotRadius, 100 + robotRadius),
        color=(0, 255, 0),
        thickness=-1,
    )
    cv2.rectangle(
        maze,
        pt1=(100 - robotRadius, 150 - robotRadius),
        pt2=(150 + robotRadius, mazeSize[1]),
        color=(0, 255, 0),
        thickness=-1,
    )
    ...
    hexBoundPts = np.array(
        [
            ...
            [
                365 + robotRadius,
                math.floor(125 - 37.5) - math.floor(robotRadius * math.sin(hexRad)),
            ],
            [
                365 + robotRadius,
                math.ceil(125 + 37.5) + math.ceil(robotRadius * math.sin(hexRad)),
            ],
            [300, 201 + robotRadius],
            [
                235 - robotRadius,
    ...
    triUpperBoundPts = np.array(
        [
            [460, 25],
            [
                460 + int(robotRadius * math.cos(triRad)),
                25 - int(robotRadius * math.sin(triRad)),
            ],
            [
                510 + int(robotRadius * math.cos(triRad)),
                125 - int(robotRadius * math.sin(triRad)),
            ],
            [510, 125],
        ]
    )
    ...
    triLowerBoundPts = np.array(
        [
            ...
            [
                460 + int(robotRadius * math.cos(triRad)),
                225 + int(robotRadius * math.sin(triRad)),
            ],
            [460, 225],
        ]
    )
...
RPM1 = int(input("Enter the velocity of the right wheel: "))
RPM2 = int(input("Enter the velocity of the left wheel: "))
"""The formula for getting the new x, y, 0 is 
x = (r/2 (RPM1 + RPM2) cos0) *dt
y = (r/2 (RPM1 + RPM2) sin0) *dt
0 = (r/L (RPM2 - RPM1)) * dt
r = clearnce
L = Wheel distance as per the robot datasheet
I used r = 0.038 and L = 0.354 as given in the cost function """
...
def actup(node, maze):
    ...
    dt = 0.1
    xup = roundCoord(
        node[0][0] + (0.5 * r * (RPM1 + RPM1) * math.cos(math.radians(node[1])) * dt)
    )
    yup = roundCoord(
        node[0][0] + (0.5 * r * (RPM1 + RPM1) * math.sin(math.radians(node[1])) * dt)
    )
    angup = checkAngle(node[1]) + (((r / L) * (RPM2 - RPM1)) * dt)

    if checkObstacle((xup, yup), maze) == False:
        newThetaup = [None, None, ((xup, yup), angup), None]
        return newThetaup
    else:
        return None

def actdown(node, maze):
    r = 0.038
    L = 0.354

    dt = 0.1
    xdown = roundCoord(
        node[0][0] + (0.5 * r * (RPM2 + RPM2) * math.cos(math.radians(node[1])) * dt)
    )
    ydown = roundCoord(
        node[0][0] + (0.5 * r * (RPM2 + RPM2) * math.sin(math.radians(node[1])) * dt)
    )
    angdown = checkAngle(node[1]) + (((r / L) * (RPM2 - RPM1)) * dt)

    if checkObstacle((xdown, ydown), maze) == False:
        newThetadown = [None, None, ((xdown, ydown), angdown), None]
        return newThetadown
    else:
        return None

def actright(node, maze):
    r = 0.038
    L = 0.354

    dt = 0.1
    xright = roundCoord(
        node[0][0] + (0.5 * r * (RPM1 + 0) * math.cos(math.radians(node[1])) * dt)
    )
    yright = roundCoord(
        node[0][0] + (0.5 * r * (RPM1 + 0) * math.sin(math.radians(node[1])) * dt)
    )
    angright = checkAngle(node[1]) + (((r / L) * (RPM2 - RPM1)) * dt)

    if checkObstacle((xright, yright), maze) == False:
        newThetaup = [None, None, ((xright, yright), angright), None]
        return newThetaup
    else:
        return None

def actleft(node, maze):
    r = 0.038
    L = 0.354

    dt = 0.1
    xleft = roundCoord(
        node[0][0] + (0.5 * r * (RPM2 + 0) * math.cos(math.radians(node[1])) * dt)
    )
    yleft = roundCoord(
        node[0][0] + (0.5 * r * (RPM2 + 0) * math.sin(math.radians(node[1])) * dt)
    )
    angleft = checkAngle(node[1]) + (((robotRadius / L) * (RPM2 - RPM1)) * dt)

    if checkObstacle((xleft, yleft), maze) == False:
        newThetaleft = [None, None, ((xleft, yleft), angleft), None]
        return newThetaleft
    else:
        return None

def searchNode(nodeCoords, maze):
    up = actup(nodeCoords, maze)
    down = actdown(nodeCoords, maze)
    right = actright(nodeCoords, maze)
    left = actleft(nodeCoords, maze)
    ...
    for i in results:
        if graph[int(2 * i[2][0][0])][int(2 * i[2][0][1])][int(i[2][1] / 30)] == 0:
            ...
            if costDict[tempIndex] > first[3] + stepSize:
                ...
                costDict[tempIndex] = (
                    first[3] + stepSize + euclideanCostToGo(i[2][0], goal[0])
                )
